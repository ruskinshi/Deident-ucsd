...
class MainCanvas(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.cancel_button = QtWidgets.QPushButton(self.tr("Exit"))
        self.finish_button = QtWidgets.QPushButton(self.tr("&Deidentify"))
        self.help_button = QtWidgets.QPushButton(self.tr("Help"))
...
class DisplayMessage(QtWidgets.QDialog):
    def __init__(self, parent=None, message=None):
        QtWidgets.QDialog.__init__(self, parent)

        self.setWindowTitle(self.tr("Display a long message"))
        
        self.exit_button = QtWidgets.QPushButton(self.tr("Exit"))
        ...
        # Show the message
        #QtWidgets.QPlainTextEdit(message)
        msg_area = QtWidgets.QTextBrowser()
...
class Features(QtWidgets.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.top_label = QtWidgets.QLabel(self.tr("<center>"
        ...
        self.existing_directory_label = QtWidgets.QLabel()
        self.existing_directory_label.setFrameStyle(frameStyle)
        self.existing_directory_label.setText(self.default_dir)
        self.existing_directory_button = QtWidgets.QPushButton(self.tr("Set Folder Containing Subdirectories with Identified Existing Original DICOM Files"))
        
    
        # Specify the output for the deidentified images
        self.new_directory_label = QtWidgets.QLabel()
        self.new_directory_label.setFrameStyle(frameStyle)
        self.new_directory_label.setText(self.output_dir)
        self.new_directory_button = QtWidgets.QPushButton(self.tr("Set Folder for Output Structure"))

        #New Generate Random Identifier Button
        self.random_number_button = QtWidgets.QPushButton(self.tr("Generate new Identifiers (Optional)"))

        # Specify how many/which images to de-ident
        self.num_images_label = QtWidgets.QLabel()
        self.set_which_images_label(self.get_num_images())
        self.num_images_line_edit = QtWidgets.QLineEdit()
        ...
        self.num_images = QtWidgets.QLabel()
        ...
        self.user_id_line_edit = QtWidgets.QLabel()
        ...
        #self.memory_line_edit = QtWidgets.QLineEdit()
        #self.memory_label.setBuddy(self.memory_line_edit)         
        # Set the default value
        #self.memory_line_edit.setText('800m')

        #self.deident_single = QtWidgets.QRadioButton('Deidentify Single Patient')
        #self.deident_batch = QtWidgets.QRadioButton('Deidentify Batch')
    ...
    def generate_random_number(self):
        num,msgBox = QtWidgets.QInputDialog.getInt(self,"integer input dialog","Enter Number of Patient Identifiers to Create")        
        
        if num >= 0:
            try:
                xls = pd.ExcelFile('CAP Patient list template.xlsx', engine='openpyxl')
                df1 = pd.read_excel(xls,'Sheet1',index_col=None)
                df2 = pd.read_excel(xls,'Sheet2',index_col=None)
            except IOError:
    ...
    def accept(self):
        ...
        for x in d:
            ...
            for idx,j in enumerate(d[x]):
                ...
                '''directory = QtWidgets.QFileDialog.getExistingDirectory(self,'',self.new_directory_label.text(), QtWidgets.QFileDialog.DontResolveSymlinks | QtWidgets.QFileDialog.ShowDirsOnly)
                ...
                #progress = QtWidgets.QProgressDialog("de-identifying files...", 
                #                                 "Abort", 
                #                                 100, 
                #                                 100, 
                #                                 self)
                ...
                #QtWidgets.qApp.processEvents() # this would get the bar to display, just not move
                ...
                #Format the name
                try:
                    reg = re.split('[  ^]',str(name).lower().title())
                    if len(reg) == 2:
                        name = reg[0] + ', ' + reg[1]
                    else:
