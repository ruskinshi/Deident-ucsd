...
class Features(QtGui.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.num_random_nums = QtGui.QLineEdit()
        ...
        layout.addWidget(self.deident_single,1,0)
        layout.addWidget(self.deident_batch,1,1)
        ...
        layout.addWidget(self.num_random_nums,1,2)
    ...
    def btnstate(self):
        ...
        if self.deident_batch.isChecked():
            self.random_number_button.setVisible(True)
    ...
    def generate_random_number(self):
        if self.num_random_nums.text() != '':
            try:
                num = int(self.num_random_nums.text())
                ...
                utils.display_gui_message(self, "De-identification Notice.", "Please Enter a Number")
    ...
    def accept(self):
        ...
        '''
        if self.deident_single.isChecked():
            if (self.num_existing_images) == 0:
                utils.display_gui_message(self, "De-identification Notice.", "No Files Found in This Directory!")
                return
        ...
        for x in d:
            ...
            for idx,j in enumerate(d[x]):
                ...
                ##progress.update()
                #progress.cancel()
                copy = pd.ExcelWriter('Backups\CAP Patient list template-Backup-%s.xlsx'%((datetime.now().strftime('%Y-%m-%d %H-%M-%S'))))
